...
export const validateResponse = (question, answer, confidenceScore = 0.7) => {
  // More lenient validation - check for robotics/competition related terms
  const roboticsTerms = [
    'robot', 'competition', 'team', 'challenge', 'ftc', 'first', 'game', 'match',
    'autonomous', 'teleop', 'programming', 'sensor', 'motor', 'build', 'design',
    'engineering', 'tournament', 'alliance', 'scoring', 'points', 'mechanism'
  ];

  const hasRelevantContent = roboticsTerms.some(term =>
    answer.toLowerCase().includes(term)
  ) || answer.length > 50; // Accept longer responses as likely valid

  // Much more permissive - only reject very short or clearly off-topic responses
  if (answer.length < 10 || (!hasRelevantContent && answer.length < 30)) {
    return {
      valid: false,
  ...
  return {
    valid: true,
    message: answer,
    confidence: Math.max(confidenceScore, 0.8) // Boost confidence for passed responses
  };
};
