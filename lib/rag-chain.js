    ...
    ragChain = {
      invoke: async (question) => {
        try {
          ...
          // Prepare prompt with proper message structure
          const docs = retrieveKnowledge(question, 3);
          const context = formatDocs(docs);

          // Use proper chat message format
          const messages = [
            {
              role: "system",
              content: "You are an expert FTC (FIRST Tech Challenge) mentor with deep knowledge of robotics competitions. Provide helpful, accurate answers about FTC rules, robot design, programming, and competition strategies."
            },
            {
              role: "user",
          ...
          const answer = result?.content || result || 'I apologize, but I was unable to generate a response.';
