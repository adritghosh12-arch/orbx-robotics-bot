...
const initGroq = () => {
  ...
  return new ChatGroq({
    apiKey: groqApiKey,
    model: 'llama3-8b-8192', // Use 'model' instead of 'modelName'
    temperature: 0.3,
    maxTokens: 500,
  });
};
    ...
    ragChain = {
      invoke: async (question) => {
        try {
          ...
          // Prepare prompt - simplified for testing
          const docs = retrieveKnowledge(question, 3);
          const context = formatDocs(docs);

          const simplePrompt = `You are an FTC expert. Answer this question about FIRST Tech Challenge based on the context below.
...
Answer:`;
          ...
          console.log('Calling LLM with prompt length:', simplePrompt.length);
          // Call LLM with simple string prompt
          const result = await llm.invoke(simplePrompt);
